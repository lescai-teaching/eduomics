...
workflow SUBSET_REFERENCES_TO_TARGETS {
    ...
    ch_gnomad        // channel: [ path(gnomad_vcf), path(gnomad_vcf_idx) ]
    ch_mills         // channel: [ path(mills_vcf),  path(mills_vcf_idx)  ]
    ch_1000g         // channel: [ path(1000g_vcf),  path(1000g_vcf_idx)  ]
    ch_dbsnp         // channel: [ path(dbsnp_vcf),  path(dbsnp_vcf_idx)  ]
    ...
    emit:
    target_fa               = SAMTOOLS_FAIDX_SUBSET.out.fa.collect()                                 // channel: [ val(meta), path(fasta)                  ]
    target_fai              = SAMTOOLS_FAIDX_INDEX.out.fai.collect()                                 // channel: [ val(meta), path(fai)                    ]
    target_sizes            = SAMTOOLS_FAIDX_SIZES.out.sizes.collect()                               // channel: [ val(meta), path(sizes)                  ]
    target_bwa_index        = BWA_INDEX.out.index.collect()                                          // channel: [ val(meta), path(bwa_index)              ]
    capture_bed_gz          = SUBSETCAPTURE.out.capture_bed_gz.map    { meta, bed -> bed }.collect() // channel: [ path(capture_bed_gz)                    ]
    capture_bed_index       = SUBSETCAPTURE.out.capture_bed_index.map { meta, bed -> bed }.collect() // channel: [ path(capture_bed_index)                 ]
    target_bed              = SUBSETCAPTURE.out.target_bed.map        { meta, bed -> bed }.collect() // channel: [ path(target_bed)                        ]
    target_bed_pad50        = SUBSETCAPTURE.out.target_bed_pad50.map  { meta, bed -> bed }.collect() // channel: [ path(target_bed_pad50)                  ]
    target_bed_pad500       = SUBSETCAPTURE.out.target_bed_pad500.map { meta, bed -> bed }.collect() // channel: [ path(target_bed_pad500)                 ]
    target_gnomad_vcf       = GATK4_SELECTVARIANTS_GNOMAD.out.vcf.map { meta, vcf -> vcf }.collect() // channel: [ path(vcf)                               ]
    target_gnomad_tbi       = GATK4_SELECTVARIANTS_GNOMAD.out.tbi.map { meta, tbi -> tbi }.collect() // channel: [ path(tbi)                               ]
    target_mills_vcf        = GATK4_SELECTVARIANTS_MILLS.out.vcf.map  { meta, vcf -> vcf }.collect() // channel: [ path(vcf)                               ]
    target_mills_tbi        = GATK4_SELECTVARIANTS_MILLS.out.tbi.map  { meta, tbi -> tbi }.collect() // channel: [ path(tbi)                               ]
    target_1000g_vcf        = GATK4_SELECTVARIANTS_1000G.out.vcf.map  { meta, vcf -> vcf }.collect() // channel: [ path(vcf)                               ]
    target_1000g_tbi        = GATK4_SELECTVARIANTS_1000G.out.tbi.map  { meta, tbi -> tbi }.collect() // channel: [ path(tbi)                               ]
    target_dbsnp_vcf        = GATK4_SELECTVARIANTS_DBSNP.out.vcf.map  { meta, vcf -> vcf }.collect() // channel: [ path(vcf)                               ]
    target_dbsnp_tbi        = GATK4_SELECTVARIANTS_DBSNP.out.tbi.map  { meta, tbi -> tbi }.collect() // channel: [ path(tbi)                               ]
    clinvar_benign_vcf      = SUBVAR.out.benign_vcf.collect()                                        // channel: [ val(meta), path(benign_vcf)             ]
    clinvar_pathogenic_vcf  = SUBVAR.out.pathogenic_vcf.collect()                                    // channel: [ val(meta), path(pathogenic_vcf)         ]
    clinvar_selected_vcf    = SUBVAR.out.selected_vcf.collect()                                      // channel: [ val(meta), path(selected_vcf)           ]
    target_dict             = GATK4_CREATESEQUENCEDICTIONARY.out.dict.collect()                      // channel: [ val(meta), path(dict)                   ]
    versions                = ch_versions                                                            // channel: [ versions.yml                            ]
    dna_bundle              = ch_dna_bundle.collect()                                                // channel: [ val(meta), path [all references bundle] ]
}
